(*exact replica of lab031 with extra spaces and indents*)
...
let fifth x =
  ...
  | h::t
    ...
    -> if List.length x <5 then 0 else List.nth x 5
...
let rec take n x y size =
  ...
  |         h::t ->         if n = size then y


    else take n t (List.append y [h]) (size + 1)
